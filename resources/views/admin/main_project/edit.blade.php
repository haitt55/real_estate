...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-12 text-right">
            <a href="{{ route('admin.image.create') }}?project_id={{ $mainProject->id }}" class="btn btn-info"><i class="fa fa-list"></i> Kho ảnh</a>
            <a href="{{ route('admin.main_project.index') }}" class="btn btn-success"><i class="fa fa-list"></i> Danh sách dự án</a>
        </div>
    </div>
    <br />
    <div class="row">
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-body">
                    <div class="row">
                        <div class="col-lg-12">
                            <form method="POST" action="{{ route('admin.main_project.update', $mainProject->id) }}" role="form" enctype="multipart/form-data">
                                @include('admin.layouts.partials.errors')
                                {!! csrf_field() !!}
                                {!! method_field('put') !!}
                                <div class="form-group">
                                    <label for="title">Tên dự án <span class="required">(*)</span></label>
                                    <input type="text" name="project_name" id="project_name" class="form-control" value="{{ old('project_name', $mainProject->project_name) }}">
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="row" style="margin-top: 10px;">
                                        <div class="display-image col-md-12">
                                            <img class="thumbnail" style="max-width: 200px;" id="project_image_logo_preview" src="{{ $mainProject->project_image_logo ? asset($mainProject->project_image_logo) : asset(config('custom.no_image')) }}" alt="">
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="row" style="margin-top: 10px;">
                                        <div class="display-image col-md-12">
                                            <img class="thumbnail" style="max-width: 200px;" id="project_image_header_preview" src="{{ $mainProject->project_image_header ? asset($mainProject->project_image_header) : asset(config('custom.no_image')) }}" alt="">
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="row" style="margin-top: 10px;">
                                        <div class="display-image col-md-12">
                                            <img class="thumbnail" style="max-width: 200px;" id="project_image_ads_preview" src="{{ $mainProject->project_image_ads ? asset($mainProject->project_image_ads) : asset(config('custom.no_image')) }}" alt="">
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="row" style="margin-top: 10px;">
                                        <div class="display-image col-md-12">
                                            <img class="thumbnail" style="max-width: 200px;" id="project_image_ads1_preview" src="{{ $mainProject->project_image_ads1 ? asset($mainProject->project_image_ads1) : asset(config('custom.no_image')) }}" alt="">
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <textarea name="description" id="description">{{ old('description', $mainProject->description) }}</textarea>
                                </div>
                                <div class="form-group">
                                    <label for="page_title">Page Title</label>
                                    <input type="text" name="page_title" id="page_title" class="form-control" value="{{ old('page_title', $mainProject->page_title) }}">
                                </div>
                                <div class="form-group">
                                    <label for="meta_keyword">Meta Keyword</label>
                                    <input type="text" name="meta_keyword" id="meta_keyword" class="form-control" value="{{ old('meta_keyword', $mainProject->meta_keyword) }}">
                                </div>
                                <div class="form-group">
                                    <label for="meta_description">Meta Description</label>
                                    <input type="text" name="meta_description" id="meta_description" class="form-control" value="{{ old('meta_description', $mainProject->meta_description) }}">
                                </div>
                                <div class="form-group">
                                    <div class="checkbox">
                                        <label><input type="checkbox" name="is_current" id="is_current" value="1"{{ old('is_current', $mainProject->is_current) ? ' checked="checked"' : '' }}> Đặt là dự án đang chạy</label>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <div class="col-md-1"><button type="submit" class="btn btn-primary">Lưu</button></div>
                                    <div class="col-md-1">
                                        <button style="@if($mainProject->is_current)pointer-events:none; opacity: 0.6; @endif"
                                                class="btn btn-danger" id="btn-delete" data-link="{{ route('admin.main_project.destroy', $mainProject->id) }}"><i class="fa fa-remove"></i> Xóa dự án</button></div>
                                </div>
                            </form>
                        </div>
                    </div>
                ...
                if (confirm('Do you really want to delete this data?')) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            if (data.error) {
                                window.location.href = '{{ URL::route('admin.main_project.edit', $mainProject->id) }}';
                            } else {
